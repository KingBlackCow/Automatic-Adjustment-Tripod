...
public class TensorFlowObjectDetectionAPIModel implements Classifier {
  ...
  public List<Recognition> recognizeImage(final Bitmap bitmap) {
    ...
    for (int i = 0; i < outputScores.length; ++i) {
      ...
      if(labels.get((int)outputClasses[i]).equals("person")) {
        pq.add(
                new Recognition("" + i, "person " + (int)detection.left + " " + (int)detection.right + " " + (int)detection.bottom + " " +(int)detection.top, outputScores[i], detection));
        width_1 = (int)(detection.left + detection.right) / 2;
        height_1 =  (int)(detection.bottom) ;
      }


      //pq.add(
          //new Recognition("" + i, labels.get((int)outputClasses[i]).toString(), outputScores[i], detection));
    }
//      if(pq.size() == 0)
//      {
//        width_1 = 0;
